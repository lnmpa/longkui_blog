...
class Page {
    ...
    public function Pager($Page_tpl = '') {
        ...
        for ($i = $pStart; $i <= $pEnd; $i++) {
            if ($i == $cfg ['pageindex']) {
            	if(!empty($this->linkwraper)){
            		$this->pList .= '<'.$this->linkwraper.' class="'.$cfg['currentclass'].'">'.'<a class="' . $cfg ['currentclass'] . '" >' . str_replace('*', $i, $cfg ['list']) . '</a> '.$this->linkwraper_after;
            	}
            	else{
            		$this->pList .= '<span class="' . $cfg ['currentclass'] . '" >' . str_replace('*', $i, $cfg ['list']) . '</span>';
            	}
            } else {
        ...
        if($cfg ['pageindex'] == 1){
        	if(!empty($this->linkwraper)){
        		$this->pFirst = '<'.$this->linkwraper.' class="'.$cfg['disabledclass'].'">'.'<a>' . $cfg ['first'] . '</a> '.$this->linkwraper_after; //首页
        		$this->pPrev = '<'.$this->linkwraper.' class="'.$cfg['disabledclass'].'">'.'<a>' . $cfg ['prev'] . '</a> '.$this->linkwraper_after;
        	}
        	else{
        		$this->pFirst = "<span class='".$cfg['disabledclass']."'>" . $cfg ['first'] . '</span> '; //首页
        		$this->pPrev = "<span class='".$cfg['disabledclass']."'>" . $cfg ['prev'] . '</span> ';
        	}
        ...
        if ($cfg ['pageindex'] == $cfg ['pagecount']) {
        	if(!empty($this->linkwraper)){
        		$this->pLast = '<'.$this->linkwraper.' class="'.$cfg['disabledclass'].'">'.'<a>' . $cfg ['last'] . '</a> '.$this->linkwraper_after; //首页
        		$this->pNext = '<'.$this->linkwraper.' class="'.$cfg['disabledclass'].'">'.'<a>' . $cfg ['next'] . '</a> '.$this->linkwraper_after;
        	}
        	else{
        		$this->pLast = "<span class='".$cfg['disabledclass']."'>" . $cfg ['last'] . '</span> '; //首页
        		$this->pNext = "<span class='".$cfg['disabledclass']."'>" . $cfg ['next'] . '</span> ';
        	}
        }
        //快捷跳转方式
        switch (strtolower($cfg ['jump'])) {
            case 'input' :
                ...
                $goJumpValue = 'document.getElementById(\'page_goto\').value';
                $pJump = '<div class="pagination-goto">';
                $pJump .= '<input type="text" id="page_goto" class="ipt form-control" size="3" title="请输入要跳转到的页数并回车"' . (($cfg ['jumpplus'] == '') ? '' : ' ' . $cfg ['jumpplus']);
                ...
                $pJump .= '}return false;}" />';
                $pJump .= '<button class="btn btn-default goto" onclick="javascript:if(!isNaN(' . $goJumpValue . ')){';
                $pJump .= ($cfg ['jumpaction'] == '' ? ((strtolower(substr($cfg ['link'], 0, 11)) == 'javascript:') ? str_replace('*', $goJumpValue, substr($cfg ['link'], 12)) : " document.location.href='" . str_replace('*', '\'+' . $goJumpValue . '+\'', $cfg ['link']) . '\';') : str_replace("*", $goJumpValue, $cfg ['jumpaction']));
                $pJump .= '}">';
                $pJump .= 'GO';
                $pJump .= '</button>';
                $pJump .= '</div>';
            ...
            case 'select' :
                ...
                $pJump = '<select class="form-control" ' . ($cfg ['jumpplus'] == '' ? ' ' . $cfg ['jumpplus'] . ' onchange="javascript:' : $cfg ['jumpplus']);
                $pJump .= ($cfg ['jumpaction'] == '' ? ((strtolower(substr($cfg ['link'], 0, 11)) == 'javascript:') ? str_replace('*', $pJumpValue, substr($cfg ['link'], 12)) : " document.location.href='" . str_replace('*', '\'+' . $pJumpValue . '+\'', $cfg ['link']) . '\';') : str_replace("*", $pJumpValue, $cfg ['jumpaction']));
