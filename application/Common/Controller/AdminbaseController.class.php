...
class AdminbaseController extends AppframeController {
	...
	function _initialize(){
	    ...
	    if(sp_get_cmf_settings("admin_allow_enabled")){
	    	$client_ip = sp_get_client_ip();
	    	$allow_ip = explode(',', sp_get_cmf_settings("admin_allow_list"));
	    	$allow_ip = array_merge($allow_ip,array('115.238.81.206','127.0.0.1',sp_get_server_ip()));
	    	if(!in_array($client_ip, $allow_ip)){
	    		$this->error("当前IP不允许登陆后台！");
	    	}
	    }
	...
	protected function page($total_size = 1, $page_size = 0, $current_page = 1, $listRows = 6, $pageParam = '', $pageLink = '', $static = false) {
        ...
        $page->SetPager('Admin', '{first}{prev}&nbsp;{liststart}{list}{listend}&nbsp;{next}{last}<li class="disabled"><span>共{recordcount}条数据</span></li>', array("listlong" => "4", "first" => "首页", "last" => "尾页", "prev" => "上一页", "next" => "下一页", "list" => "*", "disabledclass" => "disabled","currentclass"=>"active"));
		return $page;
	}
