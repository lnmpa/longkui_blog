...
class HomebaseController extends AppframeController {
	...
	function _initialize() {
		...
		if(sp_is_user_login()){
			...
			$this->assign("user",$this->users_model->where(array("id"=>$this->userid))->find());
		}
		else{
			$cookie_user = cookie('auto_login_user');
			if($cookie_user){
				...
				$this->users_model=D("Common/Users");
				$this->users_model->where(array('id'=>$cookie_user["id"]))->save($data);
				$this->userid=sp_get_current_userid();
				$this->assign("user",$this->users_model->where(array("id"=>$this->userid))->find());
			}
		}
		if(I('theme')=='mobile'){
			...
			$this->theme = 'simplebootx_mobile';
		}
		else if(I('theme')=='pc'){
			session('theme','pc');
			$this->theme = 'simplebootx';
		}
		else if(session('theme') == 'mobile'){
			$this->theme = 'simplebootx_mobile';
		}
		else if(session('theme') == 'pc'){
			$this->theme = 'simplebootx';
		}
