...
class AppframeController extends Controller {
    ...
    protected function page($totalSize = 1, $pageSize = 0, $currentPage = 1, $listRows = 6, $pageParam = '', $pageLink = '', $static = FALSE) {
    	...
    	if(sp_is_mobile()){
    	    $page->SetPager('default', '{first}{prev}{list}{next}{last}', array("listlong" => "4", "prev" => "上一页", "next" => "下一页","first" => "首页", "last" => "末页", "list" => "*","currentclass"=>"active"));
    	    $page->SetPager('mobile', '<div class="col-xs-8"><ul class="pager">{prev}{next}</ul></div><div class="col-xs-4">{jump}</div>', array("listlong" => "4", "first" => "«", "last" => "»", "prev" => "上一页", "next" => "下一页", "list" => "*", "disabledclass" => "disabled","jump"=>"select"));
    	    $page->SetPager('mobile', '{prev}{next}<form action="">{jump}', array("listlong" => "4", "prev" => "上一页", "next" => "下一页", "list" => "*", "disabledclass" => "disabled","jump"=>"select"));
    	}else{
    	    $page->SetPager('default', '{first}{prev}{list}{next}{last}', array("listlong" => "4", "prev" => "上一页", "next" => "下一页","first" => "首页", "last" => "末页", "list" => "*","currentclass"=>"active"));
    		$page->SetPager('mobile', '{prev}{next}<form action="">{jump}', array("listlong" => "4", "prev" => "上一页", "next" => "下一页", "list" => "*", "disabledclass" => "disabled","jump"=>"select"));
    	}
