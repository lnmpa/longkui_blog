...
class MenuModel extends CommonModel {
    ...
    public function admin_menu($parentid, $with_self = false) {
        ...
        $result = $this->where(array('parentid' => $parentid, 'status' => 1,'type'=>1))->order(array("listorder" => "ASC"))->select();
        if ($with_self) {
     ...
     * 获取推荐菜单
     */
    public function menu_commend() {
    	$data = $this->where(array('status'=>1,'type'=>1,'commend'=>1))->order('listorder asc')->select();
    	return $data;
    }
    ...
    public function menu_cache($data = null) {
        if (empty($data)) {
            $data = $this->where(array('status'=>1))->order('listorder asc')->select();
            F("Menu", $data);
        } else {
            F("Menu", $data);
        }
