...
class SettingController extends AdminbaseController{
	...
	public function site_post(){
		if (IS_POST) {
			...
			$cmf_settings["admin_allow_enabled"]=empty($cmf_settings['admin_allow_enabled'])?0:1;
	...
	public function contact(){
	    if(IS_POST){
	        ...
	        $data=array();
	        $data['option_name'] = "contact_setting";
	        $options=I('post.options/a');
	        $data['option_value']= json_encode($options);
	        $options_model= M('Options');
	        if($options_model->where("option_name='contact_setting'")->find()){
	            $result = $options_model->where("option_name='contact_setting'")->save($data);
	        }else{
	    ...
	    $where = array('option_name'=>'contact_setting');
