...
class MenuController extends AdminbaseController {
    ...
    public function index() {
        ...
        foreach ($result as $n=> $r) {
            ...
            $result[$n]['str_manage'] = '<a href="' . U("Menu/add", array("parentid" => $r['id'], "menuid" => I("get.menuid"))) . '" class="js-iframe-post" title="添加菜单">'.L('ADD_SUB_MENU').'</a>
            		 | <a href="' . U("Menu/edit", array("id" => $r['id'], "menuid" => I("get.menuid"))) . '" class="js-iframe-post" title="编辑菜单">'.L('EDIT').'</a>
            		 | <a class="js-ajax-delete" href="' . U("Menu/delete", array("id" => $r['id'], "menuid" => I("get.menuid")) ). '" data-callback="removeAll">'.L('DELETE').'</a> ';
            $result[$n]['status'] = $r['status'] ? 'fa-check' : 'fa-close';
            $result[$n]['type'] = $r['type'] ? 'fa-check' : 'fa-close';
            $result[$n]['commend'] = $r['commend'] ? 'fa-check' : 'fa-close';
            if(APP_DEBUG){
        ...
        $tree->init($result);
        $str = "<tr id='node-\$id' data-id='\$id' \$parentid_node style='\$style'>
					<td style='padding-left:20px;'><input name='listorders[\$id]' type='text' size='3' value='\$listorder' class='form-control input-sm input-order'></td>
					<td>\$id</td>
        			<td><fff>\$app</fff></td>
					<td>\$spacer<fff>\$name</fff></td>
				    <td style='text-align:center;'><i class='fa \$status' data-type='status'></i></td>
        			<td style='text-align:center;'><i class='fa \$type' data-type='type'></i></td>
        			<td style='text-align:center;'><i class='fa \$commend' data-type='commend'></i></td>
					<td>\$str_manage</td>
				</tr>";
    ...
    public function add() {
    	...
    	$parentid_text = $this->menu_model->where(array('id'=>$parentid))->getField('name');
    	$this->assign('parentid_text',$parentid_text);
    	
    	$this->tree_categorys = $this->_get_menu();
    	$this->display();
    }
    ...
    public function edit() {
		...
		if($rs['parentid']==0){
			$this->assign('parentid_text',"作为一级菜单");
		}
		else{
			$parentid_text = $this->menu_model->where(array('id'=>$rs['parentid']))->getField('name');
			$this->assign('parentid_text',$parentid_text);
		}
		$this->assign('parentid',$rs['parentid']);
    ...
    private function _get_menu(){
    	$retStr .= "[";
    	$retStr .= "{name:\"作为一级菜单\",\"value\":\"0\",\"click\":\"changeMenu('0','作为一级菜单')\"},";
    	$retStr .= $this->display_menu(0) . "]";
    	return $retStr;
    }
    
    private function display_menu($classid) {
    	$Mod = $this->menu_model;
    	$data['parentid'] = $classid;
    	$result = $Mod->where($data)->order(array("listorder" => "ASC"))->select();
    	$retStr = "";
    	 
    	foreach($result as $row){
    		$retStr .= "{name:\"".$row['name'] . "\"";
    		$retStr .= ",\"value\":\"".$row['id']."\",\"click\":\"changeMenu('".$row['id']."','".$row['name']."')\"";
    
    		if($row['parentid']==0)
    		{
    			$retStr .=",open:true";
    		}
    		if($this->display_menu($row['id']) != "")
    		{
    			$retStr .= ",children:[";
    			$retStr .= $this->display_menu($row['id']);
    			$retStr .= "]";
    		}
    		$retStr .= "},";
    	}
    ...
    public function status(){
    	...
    	$re = $this->menu_model->where(array('id'=>array('in',$ids)))->setField($type,$value);
    	if ($re) {
    		$this->success("更新成功！");
    	} else {
    		$this->error("更新失败！");
    ...
    public function setting(){
    	if(IS_POST){
    		$data=array();
    		$data['option_name'] = "menu_setting";
    		$options=I('post.options/a');
    		$options["is_menu_frap"]=empty($options['is_menu_frap'])?0:1;
    		$data['option_value']= json_encode($options);
    		$options_model= M('Options');
    		if($options_model->where("option_name='menu_setting'")->find()){
    			$result = $options_model->where("option_name='menu_setting'")->save($data);
    		}else{
    	...
    	$where = array('option_name'=>'menu_setting');
