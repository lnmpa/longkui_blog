...
class UserController extends AdminbaseController{
	...
	protected $UsersMod,$RoleMod;

	public function _initialize() {
		parent::_initialize();
		$this->UsersMod = D("Common/Users");
		$this->RoleMod = D("Common/Role");
	}
	...
	public function index(){
		...
		$count=$this->UsersMod->where($where)->count();
	...
	public function add(){
		if(IS_POST){
			if(!empty($_POST['role_id']) && is_array($_POST['role_id'])){
				...
				unset($_POST['role_id']);
				$_POST['user_type'] = 1;
				if ($this->UsersMod->create()!==false) {
					$result=$this->UsersMod->add();
					if ($result!==false) {
						...
						$this->error("添加失败！".$this->UsersMod->getDbError());
					}
				} else {
					$this->error($this->UsersMod->getError());
				}
			}else{
		...
		$roles=$this->RoleMod->where(array('status' => 1))->order("id DESC")->select();
		$this->assign("roles",$roles);
		$this->display();
	}
	...
	public function edit(){
		...
		$user=$this->UsersMod->where(array("id"=>$id))->find();
		$this->assign($user);
		$this->display();
	}
	...
	public function delete(){
		...
		if ($this->UsersMod->delete($id)!==false) {
	...
	public function userinfo_post(){
		if (IS_POST) {
			...
			$create_result=$this->UsersMod
			->field("id,user_nicename,avatar,sex,birthday,user_url,signature")
			->create();
			if ($create_result!==false) {
				if ($this->UsersMod->save()!==false) {
					$this->success("保存成功！");
				} else {
					$this->error("保存失败！");
				}
    ...
    public function ban(){
        $id = I('get.id',0,'intval');
    	if (!empty($id)) {
    		$result = $this->UsersMod->where(array("id"=>$id,"user_type"=>1))->setField('user_status','0');
    		if ($result!==false) {
    ...
    public function cancelban(){
    	$id = I('get.id',0,'intval');
    	if (!empty($id)) {
    		$result = $this->UsersMod->where(array("id"=>$id,"user_type"=>1))->setField('user_status','1');
    		if ($result!==false) {
