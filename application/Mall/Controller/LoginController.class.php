...
class LoginController extends HomeController {
	...
	public function index(){
	    ...
	    $this->assign('remember_login_name',cookie('remember_login_name'));
	...
	public function logout(){
		...
		cookie('auto_login_user',null);
		redirect(U('Mall/Login/index'));
	}
	...
	public function doactive(){
		...
		if($current_user['user_status']==2){
		    ...
		    $this->success('激活邮件发送成功，激活请重新登录！',U("Lndex/logout"));
	...
	public function send_mobile_code_to_resetpass(){
		$mobile = I('mobile');
		if(!$mobile){
			$this->error("手机号为空！");
		}
		$users_model=M("Users");
		$count = $users_model->where(array('mobile'=>$mobile))->count();
		if($count <= 0){
			$this->error('该手机号未注册！');
		}
		...
		if($result){
			...
			$rel = $this->_send_mobile_code($result, $mobile);
	 ...
	 * 发送短信
	 */
	private function _send_mobile_code($code,$mobile){
		$aldy=new AlidayuApi();
		...
		if(IS_POST){
			if(!sp_check_verify_code()){
				...
				if($users_model->validate($rules)->create()===false){
					...
					if($result){
						$this->success("密码重置成功，请登录！",U("Login/index"));
					}else {
    ...
    private function _do_mobile_login(){
        ...
        if(!empty($result)){
            if(sp_compare_password($password, $result['user_pass'])){
                ...
                if(I('is_remember')){
                	cookie('remember_login_name',$where['mobile'],3600*24*14);
                }
                else{
                	cookie('remember_login_name',null);
                }
                if(I('is_auto_login')){
                	cookie('auto_login_user',$result,3600*24*14);
                }
                else{
                	cookie('auto_login_user',null);
                }
                if(!empty(I('noRedirect'))){
                	$this->success("登录验证成功！");
                }
                else{
