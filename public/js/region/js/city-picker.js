...
(function (factory) {
    ...
    CityPicker.prototype = {
        ...
        render: function () {
            var p = this.getPosition(),
                ...
                dropdown = '<div class="city-picker-dropdown" style="left:0px;top:33px;' +
        ...
        getText: function () {
            ...
            this.$dropdown.find('.city-select')
                .each(function () {
                    var code = $(this).find('.active').data('code'),
						title = $(this).find('.active').attr('title'),
                        type = $(this).data('count');
                    if (code) {
                        text += ($(this).hasClass('province') ? '' : '/') + '<span class="select-item" data-count="' +
                            type + '" data-code="' + code + '">' + title + '</span>';
                    }
					else{
						return false;
					}
        ...
        output: function (type) {
			...
			code = (
                type === PROVINCE ? options.firstNode :
                    type === CITY ? this.$province && this.$province.find('.active').data('code') :
                        type === DISTRICT ? this.$city && this.$city.find('.active').data('code') : code
            )
			if($.isNumeric(code)){
				var obj = this;
				$.post(options.netUrl,{pid:code},function(data){
					districts = data.info;
					var data = [];
					if ($.isPlainObject(districts)) {
		                $.each(districts, function (code, org) {
							if (org.id == value) {
		                        matched = {
		                            code: org.id,
		                            address: org.name
		                    ...
		                    data.push({
		                        code: org.id,
		                        address: org.name,
		                        selected: org.id == value
		                    });
		                });
		            }
					
					$select.html(obj.getList(data, type));
		            $select.data('item', matched);
					var next = null;
					for(var i=0;i<obj.dems.length;i++){
						if(obj.dems[i] == type){
							next = i+1;
						}
					}
					if(obj.dems[next] != null){
						obj.output(obj.dems[next]);
					}
					else{
			            obj.feedText();
			            obj.feedVal();
		...
		getDistricts:function (code,fun){
			return fun(code);
		},
    ...
    $.fn.citypicker = function (option) {
        ...
        return this.each(function () {
            ...
            if (typeof option.behavior === 'string' && $.isFunction(fn = data[option.behavior])) {
                fn.apply(data, args);
            }
        });
    };
