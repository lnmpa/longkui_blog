;(function () {
    ...
    if (ajaxForm_list.length) {
		Wind.css('sweetalert');
        Wind.use('ajaxForm', 'sweetalert','validate', function () {
            ...
            ajaxForm_list.each(function(){
            	$(this).validate({
                	...
                	submitHandler:function(form){
						...
						var $btn = $form.find('button.js-ajax-submit');
                		$form.ajaxSubmit({
	...
	if ($('.js-ajax-get').length) {
		Wind.use('noty', function(){
			//ajax get请求
			$('.js-ajax-get').click(function(){
				var target;
				var that = this;
	...
	var $ajax_upload_img=$(".ajax-upload-img");
	if ($ajax_upload_img.length) {
		Wind.css('plupload');
		Wind.use('plupload', function(){
			$ajax_upload_img.each(function(){
				var button = $(this).attr('id');
				var multi = $(this).attr('data-multi');
				var name = $(this).attr('data-name');
				$button = $("#"+button);
				$button.wrap('<div class="img_upload_wrap"></div>');
				$button.wrap('<div id="'+button+'-container" style="float:left;"></div>');
				$("#"+button+"-container").before('<ul id="'+button+'-list" style="float:left;"></ul>');
				
				// Custom example logic
				var uploader = new plupload.Uploader({
					runtimes : 'html5,flash,silverlight,html4',
					browse_button : button, // you can pass an id...
					container: document.getElementById(button+'-container'), // ... or DOM Element itself
					url: GV.ROOT+"index.php/asset/asset/plupload",
					flash_swf_url: GV.ROOT+'public/js/plupload/Moxie.swf',
					silverlight_xap_url: GV.ROOT+'public/js/plupload/Moxie.xap',
					filters: {
						max_file_size: '5mb',
						mime_types: [{
							"title": "选择图片文件",
					...
					init: {
							...
							plupload.each(files, function(file){
								var $newfile = $('\
										<li class="wrapper-items">\
											<div class="upload-delete">删除</div>\
											<div class="upload-percent"></div>\
										</li>');
								$newfile.attr('id', file.id);
								$('#'+button+'-list').append($newfile);
								$newfile.on('click', '.upload-delete', function(){
									var $this = $(this);
									$this.parents(".wrapper-items").remove();
								});
							});
							uploader.start();
						},
						...
						UploadProgress: function(up, file) {
							$('#' + file.id).find('.upload-percent').html(file.percent + "%");
						},
						...
						FileUploaded: function(up, file, response){
							...
							if (data.status == 1) {
								...
								$('#' + file.id).addClass('uploaded').find('.upload-percent').html($img);
								
								var $input_img = $('<input type="hidden"/>');
								$input_img.attr('name', name+'[]');
								$input_img.attr('value', data.filepath);
								$('#' + file.id).find('.upload-percent').append($input_img);
							}
							else {
								alert(data.message);
								$('#' + file.id).remove();
